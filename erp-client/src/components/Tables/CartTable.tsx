...
function CartTable({ cart }:any) {
  const updateCartQuantity = void(0) as any // useUpdateCartQuantityContext()
  ...
  function updateItem(id:any, quantity:number) {
            ...
            <tr key={item.variantId} className="text-sm sm:text-base text-gray-600 text-center">
              ...
              <td className="font-primary font-medium px-4 sm:px-6 py-4">
                <input
                  ...
                  onChange={(e:any) => updateItem(item.variantId, e.target.value)}
                  className="text-gray-900 form-input border border-gray-300 w-16 rounded-sm focus:border-palette-light focus:ring-palette-light"
                />
              </td>
              <td className="font-primary text-base font-light px-4 sm:px-6 py-4 hidden sm:table-cell">
                {"$"}<span className={"text-lg"}>{item.variantPrice}</span>
              </td>
              <td className="font-primary font-medium px-4 sm:px-6 py-4">
                <button
