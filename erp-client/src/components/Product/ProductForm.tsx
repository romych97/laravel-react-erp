...
function ProductForm({ title, handle, variants, setVariantPrice, mainImg }:any) {
  const [quantity, setQuantity] = useState(1) as any
  const [variantId, setVariantId] = useState(variants[0].node.id) as any
  const [variant, setVariant] = useState(variants[0]) as any
  const isLoading = [] as any //useCartContext()[2]
  const addToCart = void(0) as any //useAddToCartContext()
